    ...
    public class Calculator
    {
        ...
        private static List<string> ConvertInfixToPostfix(List<string> tokens)// это всё говно нужно для обратной польской записи
        {
            ...
            foreach (var token in tokens) //перебираем все токены  (части выражения в виде чисел и операторов
            {
                ...
                else if (token == ")")//НАЧАЛО ИСПРАВЛЕННОЙ ЧАСТИ, Я ЗНАЮ, ЧТО ТУТ ЕСТЬ ИСТОРИЯ, ЭТО Я У СЕБЯ ДЛЯ СЕБЯ В ФАЙЛЕ ЧИСТО ПОМЕТИЛ
                { 
                    bool foundOpening = false;//булевая переменная, отвечает есть ли ( в записи или нет      
                    while (operators.Count > 0)//цикл, пока в стеке операторов есть элементы
                    {
                        var top = operators.Pop(); //извлекаем верхний элемент и проверяем далее что в нём находится
                        if (top == "(")
                        {
                            foundOpening = true;
                            break; // нашли открывающую скобку — заканчиваем обработку
                        }
                        else
                        {
                            output.Add(top);//добавляем этот оператор или символ в output
                        }
                    }
                    if (!foundOpeningParenthesis)// если не нашлась открывающая скобка, значит выбрасываем исключение
                    {
                        throw new ArgumentException("Несбалансированные скобки: отсутствует открывающая скобка для закрывающей.");
                    }
                }
            }
